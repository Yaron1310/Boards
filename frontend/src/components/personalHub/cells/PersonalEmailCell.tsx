...
const PersonalEmailCell: React.FC<PersonalCellProps> = ({ column, itemId, itemName, value, editable, userId }) => {
      ...
      {(isEditing, stopEdit) => {
        ...
        return (
          <div className="px-3 py-2 text-sm truncate w-full text-center">
            {rawValue ? (
              <a href={`mailto:${rawValue}`} className="text-indigo-600 hover:underline" onClick={(e) => e.stopPropagation()} aria-label={`Email ${rawValue}`}>
                {rawValue}
              </a>
            ) : (
...
export default React.memo(PersonalEmailCell);
