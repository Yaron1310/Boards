...
interface StatusOptionsMenuProps {
  ...
  handleDeleteLabel: (e: React.MouseEvent, optionId: string) => void;
  handleAddLabel: (e: React.MouseEvent) => void;
  isAdding: boolean;
...
const StatusOptionsMenu: React.FC<StatusOptionsMenuProps> = ({
  anchorEl, column, settings, value, select, handleDeleteLabel, handleAddLabel,
  isAdding, setIsAdding, newName, setNewName, newColor, setNewColor,
  colorPickerOpen, setColorPickerOpen, colorPickerRef, onClose,
}) => {
  const anchorRect = anchorEl?.getBoundingClientRect() ?? null;
  const { ref: menuRef, style: menuPos } = useFlippedPosition<HTMLDivElement>(anchorRect, MENU_W);
        ...
        <>
          ...
          {isEditing && (
            <StatusOptionsMenu
              ...
              settings={settings}
              value={value}
              select={(optionId) => select(optionId, stopEdit)}
              ...
              onClose={() => { setIsAdding(false); stopEdit(); }}
            />
          )}
        </>
      )}
